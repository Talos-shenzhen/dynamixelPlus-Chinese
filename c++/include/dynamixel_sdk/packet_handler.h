...
#define TI_MAKEWORD(a, b)  (static_cast<uint16_t>((static_cast<uint8_t>((static_cast<uint64_t>(a)) & 0xff)) | (static_cast<uint16_t>(static_cast<uint8_t>((static_cast<uint64_t>(b)) & 0xff))) << 8))
#define TI_MAKEDWORD(a, b) (static_cast<uint32_t>((static_cast<uint16_t>((static_cast<uint64_t>(a)) & 0xffff)) | (static_cast<uint32_t>(static_cast<uint16_t>((static_cast<uint64_t>(b)) & 0xffff))) << 16))
#define TI_LOWORD(l)       (static_cast<uint16_t>((static_cast<uint64_t>(l)) & 0xffff))
#define TI_HIWORD(l)       (static_cast<uint16_t>(((static_cast<uint64_t>(l)) >> 16) & 0xffff))
#define TI_LOBYTE(w)       (static_cast<uint8_t>((static_cast<uint64_t>(w)) & 0xff))
#define TI_HIBYTE(w)       (static_cast<uint8_t>(((static_cast<uint64_t>(w)) >> 8) & 0xff))

// 这里定义用于 TouchIdeas 的通讯包的命令字类型
enum  TouchIdeasInstruction {
...
class WINDECLSPEC PacketHandler {
 ...
 public:
    static const uint8_t _BroadCastId =0xfe;  // 广播地址254
    static const uint8_t _MaxId =0xfc;  // 设备最大地址 252

    /// @brief 这个函数根据给定的版本参数返回一个协议句柄
    /// @return PacketHandler 的对应版本的句柄
    static PacketHandler *getPacketHandler(double protocol_version = 2.0);

    virtual ~PacketHandler() { }

    static void packetHalfWord(uint8_t *data, uint16_t val) {
        data[0] =TI_LOBYTE(val);
        data[1] =TI_HIBYTE(val);
    }
    ...
    static void packetWord(uint8_t *data, int32_t val) {
        data[0] = TI_LOBYTE(TI_LOWORD(val));
        data[1] = TI_HIBYTE(TI_LOWORD(val));
        data[2] = TI_LOBYTE(TI_HIWORD(val));
        data[3] = TI_HIBYTE(TI_HIWORD(val));
    }
    ...
    static uint16_t getHalfWord(uint8_t *data) {
        return TI_MAKEWORD(data[0], data[1]);
    }

    static uint32_t getWord(uint8_t *data) {
        return TI_MAKEDWORD(TI_MAKEWORD(data[0], data[1]), TI_MAKEWORD(data[2], data[3]));
    }

    /// @brief 返回当前句柄支持的通讯协议版本
    /// @return 通讯协议版本
    virtual double getProtocolVersion() = 0;

    /// @brief 使用这个函数获取通讯错误码对应的描述字符串
    /// @param result 通讯错误码
    /// @return 错误码对应的描述字符串
    virtual const char *getTxRxResult(CommErrorCode result) = 0;
    ...
    /// @description 这个函数会调用 PortHandler::clearPort() 清空串口的发送缓存区，
    /// 然后调用 PortHandler::writePort() 发送数据包
    /// @description 只有当串口空闲，并且数据报写入发送缓存区后才会实际发送数据
    /// @param port 发送数据报使用的串口
    /// @param txpacket 要发送的命令数据报
    /// @return 如果串口忙，返回 CommPortBusy；如果数据报长度超过 TXPacketMaxLen
    /// 则返回 CommTxError；如果数据报太短，返回 CommTxFailed
    /// @return 发送成功返回 CommSuccess
    virtual CommErrorCode txPacket(PortHandler *port, uint8_t *txpacket) = 0;

    /// @brief 这个函数会尝试从指定串口接收一个状态数据报
    /// @description 这个函数会持续调用 PortHandler::readPort() 以读取数据报，
    /// 当 PortHandler::isPacketTimeout() 返回超时时它会中断循环退出；
    /// @description 或者发现数据报损毁，也会中断
    /// @param port 接收数据报使用的串口
    /// @param rxpacket 接收到的状态数据报
    /// @return 如果数据报损坏（没有头部；ID,长度，错误字非法；数据报太短），
    /// 返回 CommRxCorrupt；如果超时，返回 CommRxTimeout
    /// @return 成功接收，校验失败返回 CommRxFailed；否则返回 CommSuccess
    virtual CommErrorCode rxPacket(PortHandler *port, uint8_t *rxpacket) = 0;
    ...
    virtual CommErrorCode txRxPacket(PortHandler *port, uint8_t *txpacket,
                                     uint8_t *rxpacket, uint8_t *error =nullptr) = 0;
    ...
    virtual CommErrorCode ping(PortHandler *port, uint8_t id, uint8_t *error =nullptr) = 0;

    /// @brief 这个函数 PING 一个伺服，同时获取该设备的型号
    /// @description 这个函数会构造 InstPing 命令数据报, 然后使用 txRxPacket() 发送数据报接收响应；
    /// 成功则通过 readTxRx() 函数读取响应中的设备型号
    /// @description 如果 ID 是 _BroadCastId，那么会直接退出
    /// @param port 发送数据报使用的串口
    /// @param id 伺服的通讯地址
    /// @param error 伺服的硬件状态字
    /// @return 如果指定的地址是 _BroadCastId，则返回 CommNotImplement；发送接收成功，则返回 CommSuccess
    /// @return 其它错误则是由于其调用的 txRxPacket() 和 readTxRx() 所返回
    virtual CommErrorCode ping(PortHandler *port, uint8_t id, uint16_t *model_number, uint8_t *error =nullptr) = 0;
    ...
    /// @brief 这个命令会命令一个伺服重启，这个函数会构造 InstReboot 命令数据报,
    ///  然后使用 txRxPacket() 发送数据报接收响应
    /// @description 发送成功，那么伺服会重启，重启时可以看到它的 LED 灯闪烁
    /// @param port 发送数据报使用的串口
    /// @param id 伺服的通讯地址
    /// @param error 伺服的硬件状态字
    /// @return 如果是 1.0 版本的协议，会直接返回 CommNotImplement
    virtual CommErrorCode reboot(PortHandler *port, uint8_t id, uint8_t *error =nullptr) = 0;

    /// @brief 这个函数请求伺服将 EEPROM 参数回复到出厂设定，这个函数会构造 InstFactoryReset 命令数据报,
    ///  然后使用 txRxPacket() 发送数据报接收响应
    ...
    /// @param error 伺服的硬件状态字
    /// @return 返回 txRxPacket() 函数执行的结果错误码
    virtual CommErrorCode factoryReset(PortHandler *port, uint8_t id,
                                       uint8_t option = 0, uint8_t *error =nullptr) = 0;

    /// @brief 这个函数会请求伺服进行一次零位操作，以找到其零位，
    /// 这个函数会构造 InstHomig 命令数据报, 然后使用 txRxPacket() 发送数据报接收响应
    ...
    /// @param error 伺服的硬件状态字
    /// @return 返回 txRxPacket() 函数执行的结果错误码
    virtual CommErrorCode homing(PortHandler *port, uint8_t id,
                                 uint8_t direction = 0, uint8_t *error =nullptr) = 0;

    /// @brief 这个函数负责读取指定地址的寄存器，这个函数会构造 InstRead 命令数据报,
    ///  然后使用 txRxPacket() 发送数据报接收响应
    /// @description 如果 ID 是 _BroadCastId，那么会直接退出
    /// @param port 发送数据报使用的串口
    /// @param id 伺服的通讯地址
    ...
    /// @return 地址为 _BroadCastId，则返回 CommNotImplement；否则返回 txPacket() 函数执行的结果错误码
    ...
    virtual CommErrorCode readRx(PortHandler *port, uint16_t length,
                                 uint8_t *data, uint8_t *error =nullptr) = 0;

    /// @brief 这个函数发送一个 InstRead 指令数据报，
    /// 然后使用 txRxPacket() 发送数据报接收响应，从中读取寄存器的数据
    /// @description 如果 ID 是 _BroadCastId，那么会直接退出
    /// @param port 发送数据报使用的串口
    /// @param id 伺服的通讯地址
    /// @param address 要读取的寄存器开始地址
    ...
    /// @param error 伺服的硬件状态字
    /// @return 地址为 _BroadCastId，则返回 CommNotImplement；否则返回 txPacket() 函数执行的结果错误码
    virtual CommErrorCode readTxRx(PortHandler *port, uint8_t id, uint16_t address, uint16_t *length,
                                   uint8_t *data, uint8_t *error =nullptr) = 0;
    ...
    virtual CommErrorCode read1ByteRx(PortHandler *port, uint8_t *data, uint8_t *error =nullptr);
    ...
    /// @param error 伺服的硬件状态字
    /// @return 返回 txRxPacket() 函数执行的结果错误码
    virtual CommErrorCode read1ByteTxRx(PortHandler *port, uint8_t id,
                                        uint16_t address, uint8_t *data, uint8_t *error =nullptr);
    ...
    /// @param error 伺服的硬件状态字
    /// @return 返回 readRx() 函数执行的结果错误码
    virtual CommErrorCode read2ByteRx(PortHandler *port, uint16_t *data,
    ...
    /// @param error 伺服的硬件状态字
    /// @return 返回 txRxPacket() 函数执行的结果错误码
    virtual CommErrorCode read2ByteTxRx(PortHandler *port, uint8_t id,
                                        uint16_t address, uint16_t *data, uint8_t *error =nullptr);
    ...
    /// @param error 伺服的硬件状态字
    /// @return 返回 readRx() 函数执行的结果错误码
    virtual CommErrorCode read4ByteRx(PortHandler *port, uint32_t *data, uint8_t *error =nullptr);
    ...
    /// @param error 伺服的硬件状态字
    /// @return 返回 txRxPacket() 函数执行的结果错误码
    virtual CommErrorCode read4ByteTxRx(PortHandler *port, uint8_t id,
                                        uint16_t address, uint32_t *data, uint8_t *error=nullptr);
    ...
    virtual CommErrorCode writeTxRx(PortHandler *port, uint8_t id, uint16_t address,
                                    uint16_t length, uint8_t *data, uint8_t *error =nullptr) = 0;
    ...
    virtual CommErrorCode write1ByteTxRx(PortHandler *port, uint8_t id, uint16_t address,
                                         uint8_t data, uint8_t *error =nullptr);
    ...
    /// @param data 要写入到寄存器中的数据
    /// @return 返回 writeTxOnly() 函数执行的结果错误码
    virtual CommErrorCode write2ByteTxOnly(PortHandler *port, uint8_t id, uint16_t address, uint16_t data);
    ...
    virtual CommErrorCode write2ByteTxRx(PortHandler *port, uint8_t id, uint16_t address,
                                         uint16_t data, uint8_t *error=nullptr);
    ...
    /// @param data 要写入到寄存器中的数据
    /// @return 返回 writeTxOnly() 函数执行的结果错误码
    virtual CommErrorCode write4ByteTxOnly(PortHandler *port, uint8_t id, uint16_t address, uint32_t data);
    ...
    virtual CommErrorCode write4ByteTxRx(PortHandler *port, uint8_t id, uint16_t address,
                                         uint32_t data, uint8_t *error =nullptr);
    ...
    /// @brief 这个函数发送 InstRegWrite 指令数据报，这个命令会先让数据缓存到伺服中，
    /// 等到 InstAction 指令时才更新对应寄存器，并从状态数据报中读取硬件错误字
    /// @param port 发送数据报使用的串口
    /// @param id 伺服的通讯地址
    /// @param address 要写入的寄存器开始地址
    /// @param length 要读取的字节数
    ...
    /// @param error 伺服的硬件状态字
    /// @return 返回 txRxPacket() 函数执行的结果错误码
    virtual CommErrorCode regWriteTxRx(PortHandler *port, uint8_t id, uint16_t address,
                                       uint16_t length, uint8_t *data, uint8_t *error=nullptr) = 0;

    /// @brief （只有版本 2.0 支持）这个函数发送 InstSyncRead 指令数据报，
    /// 调用 txPacket() 发送数据报，被 GroupSyncRead 使用
    /// @param port 发送数据报使用的串口
    ...
    /// @return 返回 txPacket() 函数执行的结果错误码
    virtual CommErrorCode syncReadTx(PortHandler *port, uint16_t start_address, uint16_t data_length,
                                     uint8_t *param, uint16_t param_length) = 0;

    /// @brief 这个函数发送 InstSyncWrite 指令数据报，
    /// 调用 txRxPacket() 发送指令，但并没有处理响应数据报，被 GroupSyncWrite 使用
    /// @param port 发送数据报使用的串口
    /// @param start_address 同步读取的寄存器起始地址
    /// @param data_length 同步读取的数据字节数
    ...
    /// @return 返回 txRxPacket() 函数执行的结果错误码
    virtual CommErrorCode syncWriteTxOnly(PortHandler *port, uint16_t start_address,
                                          uint16_t data_length, uint8_t *param, size_t param_length) = 0;
