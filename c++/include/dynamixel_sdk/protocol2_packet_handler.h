...
public:
    ...
    CommErrorCode txPacket(PortHandler *port, uint8_t *txpacket);
    ...
    CommErrorCode rxPacket(PortHandler *port, uint8_t *rxpacket);

    /// @brief 这个函数从指定串口发送一个命令数据报，然后接收该数据包的响应状态数据报
    /// @description 首先它调用 txPacket() 发送数据报，如果不成功则返回错误码；
    /// 成功则调用 rxPacket() 接收状态响应
    /// @param port 发送数据报使用的串口
    /// @param txpacket 要发送的命令数据报
    /// @param rxpacket 接收到的状态数据报
    /// @return 如果发送和接收都成功，则返回 CommSuccess；否则返回 txPacket() 或 rxPacket() 返回的错误码
    CommErrorCode txRxPacket(PortHandler *port, uint8_t *txpacket,
                             uint8_t *rxpacket, uint8_t *error =nullptr);
    ...
    CommErrorCode ping(PortHandler *port, uint8_t id, uint8_t *error =nullptr);

    /// @brief 这个函数 PING 一个伺服，同时获取该设备的型号
    /// @description 这个函数会构造 InstPing 命令数据报,
    /// 然后使用 txRxPacket() 发送数据报接收响应；成功则通过 readTxRx() 函数读取响应中的设备型号
    ...
    CommErrorCode ping(PortHandler *port, uint8_t id,
                       uint16_t *model_number, uint8_t *error =nullptr);
    ...
    /// @brief 这个函数发送一个动作指令，和寄存器写指令配合，
    /// 主要用于需要多个寄存器写入的动作可以在全部指令下发后才同时更新
    ...
    /// @param error 伺服的硬件状态字
    /// @return 返回 txRxPacket() 函数执行的结果错误码
    CommErrorCode reboot(PortHandler *port, uint8_t id, uint8_t *error =nullptr);

    /// @brief 这个函数请求伺服将 EEPROM 参数回复到出厂设定，
    /// 这个函数会构造 InstFactoryReset 命令数据报, 然后使用 txRxPacket() 发送数据报接收响应
    ...
    /// @param error 伺服的硬件状态字
    /// @return 返回 txRxPacket() 函数执行的结果错误码
    CommErrorCode factoryReset(PortHandler *port, uint8_t id, uint8_t option,
                               uint8_t *error =nullptr);

    /// @brief 这个函数会请求伺服进行一次零位操作，以找到其零位，这个函数会构造 InstHomig 命令数据报,
    ///  然后使用 txRxPacket() 发送数据报接收响应
    ...
    /// @param error 伺服的硬件状态字
    /// @return 返回 txRxPacket() 函数执行的结果错误码
    CommErrorCode homing(PortHandler *port, uint8_t id,
                         uint8_t direction = 0, uint8_t *error =nullptr);
    ...
    CommErrorCode readTx(PortHandler *port, uint8_t id,
                         uint16_t address, uint16_t length);

    /// @brief 这个函数负责从状态数据报中读取寄存器数据信息，
    /// 通常前面有发送一个 InstRead 指令数据报。
    /// @param port 发送数据报使用的串口
    /// @param length 要读取的字节数
    /// @param data 从数据报中读取到的数据
    /// @param error 伺服的硬件状态字
    /// @return 返回 rxPacket() 函数执行的结果错误码
    CommErrorCode readRx(PortHandler *port, uint16_t length,
    ...
    CommErrorCode readTxRx(PortHandler *port, uint8_t id, uint16_t address,
                           uint16_t *length, uint8_t *data, uint8_t *error =nullptr);
    ...
    CommErrorCode writeTxOnly(PortHandler *port, uint8_t id, uint16_t address,
    ...
    /// @param error 伺服的硬件状态字
    /// @return 返回 txRxPacket() 函数执行的结果错误码
    CommErrorCode writeTxRx(PortHandler *port, uint8_t id, uint16_t address,
                            uint16_t length, uint8_t *data, uint8_t *error =nullptr);

    /// @brief 这个函数发送 InstRegWrite 指令数据报，这个命令会先让数据缓存到伺服中，
    /// 等到 InstAction 指令时才更新对应寄存器
    /// @param port 发送数据报使用的串口
    /// @param id 伺服的通讯地址
    /// @param address 要写入的寄存器开始地址
    /// @param length 要读取的字节数
    /// @param data 要写入到寄存器中的数据
    /// @return 返回 txPacket() 函数执行的结果错误码
    CommErrorCode regWriteTxOnly(PortHandler *port, uint8_t id, uint16_t address,
                                 uint16_t length, uint8_t *data);
    ...
    /// @param error 伺服的硬件状态字
    /// @return 返回 txRxPacket() 函数执行的结果错误码
    CommErrorCode regWriteTxRx(PortHandler *port, uint8_t id, uint16_t address,
                               uint16_t length, uint8_t *data, uint8_t *error =nullptr);
    ...
    /// @return 返回 txPacket() 函数执行的结果错误码
    CommErrorCode syncReadTx(PortHandler *port, uint16_t start_address,
                             uint16_t data_length, uint8_t *param, uint16_t param_length);

    /// @brief 这个函数发送 InstSyncWrite 指令数据报，调用 txRxPacket() 发送指令，
    /// 但并没有处理响应数据报，被 GroupSyncWrite 使用
    /// @param port 发送数据报使用的串口
    /// @param start_address 同步读取的寄存器起始地址
    /// @param data_length 同步读取的数据字节数
    ...
    /// @return 返回 txRxPacket() 函数执行的结果错误码
    CommErrorCode syncWriteTxOnly(PortHandler *port, uint16_t start_address,
                                  uint16_t data_length, uint8_t *param, size_t param_length);
...
private:
    ...
    static const uint16_t TXPacketMaxLen =(4096);    // 发送数据报的最大长度
    static const uint16_t RXPacketMaxLen =(4*1024);    // 接收数据报的最大长度
    static const uint16_t PingStatusLength = 14;           // 响应 PING 指令的状态数据报

    // 指令数据报公共部分的长度，10 字节：HEADER0 HEADER1 HEADER2 RESERVED ID LEN_L LEN_H INST （...）CRC16_L CRC16_H
    static const uint16_t CmdCommonLength  =10;

    // PING 执行指令数据报公共部分的长度，3 字节：INST CRC16_L CRC16_H
    static const uint8_t PingCmdParamLen  =3;
    static const uint16_t PingStatusLen  =3;

    // 延时写执行指令数据报公共部分的长度，3 字节：INST CRC16_L CRC16_H
    static const uint16_t ActionCmdParamLen  =3;

    // 重启指令数据报公共部分的长度，3 字节：INST CRC16_L CRC16_H
    static const uint16_t RebootCmdParamLen  =3;

    // 回复出厂设定指令数据报公共部分的长度，4 字节：INST DIRECTION CRC16_L CRC16_H
    static const uint16_t FactoryResetCmdParamLen  =4;

    // 归零指令数据报公共部分的长度，4 字节：INST DIRECTION CRC16_L CRC16_H
    static const uint16_t HomeCmdParamLen  =4;
