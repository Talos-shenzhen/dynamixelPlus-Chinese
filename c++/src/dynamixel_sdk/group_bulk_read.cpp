...
void GroupBulkRead::makeParam() {
  ...
  if (ph_->getProtocolVersion() == 1.0) { // 1.0 版本一个节点占用 3 字节描述
  ...
  for (unsigned int i = 0; i < id_list_.size(); i++) {
    ...
    if (ph_->getProtocolVersion() == 1.0) {
      ...
      param_[idx++] = id;                               // ID
      param_[idx++] = TI_LOBYTE(address_list_[id]);    // ADDR_L
      param_[idx++] = TI_HIBYTE(address_list_[id]);    // ADDR_H
      param_[idx++] = TI_LOBYTE(length_list_[id]);     // LEN_L
      param_[idx++] = TI_HIBYTE(length_list_[id]);     // LEN_H
    }
  }
}
...
uint32_t GroupBulkRead::getData(uint8_t id, uint16_t address, uint16_t data_length) {
  ...
  switch(data_length) {
    ...
    case 2:
      return TI_MAKEWORD(data_list_[id][address - start_addr], data_list_[id][address - start_addr + 1]);
    case 4:
      return TI_MAKEDWORD(TI_MAKEWORD(data_list_[id][address - start_addr + 0], data_list_[id][address - start_addr + 1]),
                           TI_MAKEWORD(data_list_[id][address - start_addr + 2], data_list_[id][address - start_addr + 3]));
    default:
      return 0;
  }
}
