...
void GroupBulkWrite::makeParam() {
  ...
  for (unsigned int i = 0; i < id_list_.size(); i++) {
    ...
    param_[idx++] = id;
    param_[idx++] = TI_LOBYTE(address_list_[id]);
    param_[idx++] = TI_HIBYTE(address_list_[id]);
    param_[idx++] = TI_LOBYTE(length_list_[id]);
    param_[idx++] = TI_HIBYTE(length_list_[id]);
...
CommErrorCode GroupBulkWrite::txPacket() {
  if (ph_->getProtocolVersion() == 1.0 || id_list_.size() == 0)
